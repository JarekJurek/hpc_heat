...
"""
To read the profiler output of script task9.py, run the following command:
nsys stats measurements/task10/task10_prof.nsys-rep

To read the profiler output of script task10.py, run the following command:
nsys stats measurements/task10/task10_prof_fixed.nsys-rep
"""
...
def jacobi(u, interior_mask, max_iter, atol=1e-6):
    ...
    # TASK 10
    # Check convergence only every CHECK_FREQ iterations to reduce device-to-host transfers 
    CHECK_FREQ = 50
    
    for i in range(0, max_iter, CHECK_FREQ):
        for j in range(min(CHECK_FREQ, max_iter - i)):
            # Compute average of left, right, up and down neighbors, see eq. (1)
            u_new = 0.25 * (u[1:-1, :-2] + u[1:-1, 2:] + u[:-2, 1:-1] + u[2:, 1:-1])
            u_new_interior = u_new[interior_mask]
            u[1:-1, 1:-1][interior_mask] = u_new_interior
        
        # Only check for convergence after CHECK_FREQ iterations
        delta = cp.abs(u_new_interior - u[1:-1, 1:-1][interior_mask]).max()
        if delta.item() < atol:  #Use .item() to get the scalar value from the array
            break
...
if __name__ == '__main__':
    ...
    csv_filename = 'building_summary_stats.csv'    
    with open(csv_filename, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['building_id'] + stat_keys)
        for bid, u, interior_mask in zip(building_ids, all_u, all_interior_mask):
            stats = summary_stats(u, interior_mask)
            csvwriter.writerow([bid] + [stats[k] for k in stat_keys])
